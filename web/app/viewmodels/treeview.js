define(['services/logger', "knockout", 'durandal/app', 'services/dataServices', "services/application", "viewmodels/mapOl3", 'viewmodels/nav', 'services/codeLists', "services/knockoutExtensions", "bootstrap-treeview", 'services/resource', 'services/config'],
    function (logger, ko, app, dataServices, application, map, nav, codeLists, jqAutoComplete, treeview, resource, config) {
        var title = "treeview",
            ...
            vm = {
                ...
                hasNodesSelected: ko.computed(function () {
                    return (application.filter.NatureLevelCodes().length +
                        application.filter.NatureAreaTypeCodes().length +
                        application.filter.DescriptionVariableCodes().length +
                        application.filter.Municipalities().length +
                        application.filter.Counties().length) +
                        application.filter.ConservationAreas().length +
                        application.filter.RedlistCategories().length +
                        application.filter.RedlistAssessmentUnits().length > 0;
                }),
                                            ...
                                            function buildTreeRecursive2(node, list, key, type) {
                                                ...
                                                if (list instanceof Array) {
                                                    _.each(list[key].countsByAssessmentUnit,
                                                        function (value, key, list) {
                                                            var url = openUrl(value.url);
                                                            var subnode = makeNode(value.name, type, value.id, url, value.count);
                                            ...
                                            function clearFilterByType(type) {
                                                switch (type) {
                                                    ...
                                                    case "variables":
                                                        ...
                                                        break;
                                                    case "redlist":
                                                        application.removeFilter("RedlistCategories");
                                                        break;
                                                    case "theme":
                                                        application.removeFilter("RedlistAssessmentUnits");
                                                        break;
                                                }
                                            }
        ...
        vm.clearCheckedNodes = function () {
            ...
            application.filter.RedlistCategories([]);
            application.filter.RedlistAssessmentUnits([]);
